...
public class AddBadgeCommand implements CommandExecutor, Listener {

	private PlayerManager plugin;


	public AddBadgeCommand(PlayerManager pl) {
		this.plugin = pl;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (args.length == 2) {
			...
			if (serverPlayer != null) {
				if (sender.hasPermission("playermanager.addbadge")) {
					serverPlayer.addBadge(args[1]);
					plugin.sendMessage(sender, "&7" + serverPlayer.getName() + "&a, player badge added.");
					return true;
				} else {
